...
class IPO:
    ...
    def addPricesToDictionary(self, sortedList, cutoffs):
        ...
        self.allPlayers = collections.OrderedDict(
            self.getSortedTuplesFromDictionary(self.allPlayers))

    # Parse CSV file into dictionary
    def parseDataDictionaryFromAPI(self, allPlayers):
        playerStats = {}

        # add all players to dictionary
        for row in allPlayers:
            # if they are projecred more than 0
            if float(row["FantasyPointsPPR"]) > 0.0:
                playerStats[row["Name"]] = float(row["FantasyPointsPPR"])
            else:
        ...
        sortedList.reverse()
        # get cutoffs
        self.addPricesToDictionary(
            sortedList, self.determineCutoffs(sortedList))
    ...
    def __init__(self, allPlayers):
        self.allPlayers = {}
        self.parseDataDictionaryFromAPI(allPlayers)
